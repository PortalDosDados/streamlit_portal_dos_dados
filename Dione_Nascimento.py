...
# Nota: O set_page_config deve ser obrigatoriamente o primeiro comando Streamlit.
# ==============================================================================
st.set_page_config(
...
# ==============================================================================
# 2. INICIALIZAÇÃO DO MONITORAMENTO E LÓGICA DA APLICAÇÃO
# O gerenciador de contexto 'streamlit_analytics.track()' encapsula toda a
# interface para garantir o rastreamento de visualizações e interações.
# ==============================================================================
with streamlit_analytics.track():
    ...
    def load_css(file_path: str):
        ...
        Args:
            file_path (str): Caminho relativo ou absoluto do arquivo .css.
        """
        try:
            ...
            # Em produção, considerar logging do erro ao invés de exibir ao usuário.
    ...
    with st.container():
        ...
        with col_foto:
            try:
                # Renderização da foto de perfil com ajuste de largura ao container
                st.image('./assets/minha_foto.png', use_container_width=True)
            except Exception:
                st.warning("Imagem de perfil não disponível.")

        with col_bio:
            # Apresentação profissional utilizando HTML para formatação avançada
            st.markdown("""
    ...
    try:
        ...
        pass  # Falha silenciosa caso o banner não exista, mantendo a fluidez da UX
